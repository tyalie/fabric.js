...
export type TModificationEvents = 'moving' | 'scaling' | 'rotating' | 'skewing' | 'resizing';
export interface ModifiedEvent<E extends Event = TPointerEvent> extends TEvent<E> {
    transform: Transform;
    target: FabricObject;
    action: string;
}
type ModificationEventsSpec<Prefix extends string = '', Modification = BasicTransformEvent, Modified = ModifiedEvent | never> = Record<`${Prefix}${TModificationEvents}`, Modification> & Record<`${Prefix}modified`, Modified>;
type ObjectModificationEvents = ModificationEventsSpec;
type CanvasModificationEvents = ModificationEventsSpec<'object:', BasicTransformEvent & {
    target: FabricObject;
}, ModifiedEvent | {
    target: FabricObject;
}> & {
...
type TPointerEvents<Prefix extends string> = Record<`${Prefix}${WithBeforeSuffix<'down'> | WithBeforeSuffix<'move'> | WithBeforeSuffix<'up'> | 'dblclick'}`, TPointerEventInfo> & Record<`${Prefix}wheel`, TPointerEventInfo<WheelEvent>> & Record<`${Prefix}over`, TPointerEventInfo & InEvent> & Record<`${Prefix}out`, TPointerEventInfo & OutEvent>;
